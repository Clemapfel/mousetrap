...
namespace mousetrap
{
    ...
    std::vector<uint64_t> SelectionModel::get_selection()
    {
        auto* bitset = gtk_selection_model_get_selection(operator GtkSelectionModel*());
        std::vector<uint64_t> out;
        for (uint64_t i = 0; i < gtk_bitset_get_size(bitset); ++i)
    ...
    void SelectionModel::select(uint64_t i, bool unselect_others)
    {
        gtk_selection_model_select_item(operator GtkSelectionModel*(), i, unselect_others);
    }

    void SelectionModel::unselect(uint64_t i)
    {
    ...
    uint64_t SelectionModel::get_n_items() const
    {
