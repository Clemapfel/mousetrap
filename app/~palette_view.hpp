...
namespace mousetrap
{
    ...
    class PaletteView : public AppComponent
    {
        ...
        private:
            ...
            Box palette_locked_box = Box(GTK_ORIENTATION_HORIZONTAL);
            SeparatorLine palette_locked_spacer;
            CheckButton palette_locked_check_button;
            static void on_palette_locked_check_button_toggled(CheckButton*, PaletteView* instance);

            float tile_size;

            Box tile_size_box = Box(GTK_ORIENTATION_HORIZONTAL);
            SeparatorLine tile_size_spacer;
            Scale tile_size_scale = Scale(2, 256, 1);
            static void on_tile_size_scale_value_changed(Scale*, PaletteView* instance);
...
namespace mousetrap
{
    ...
    void PaletteView::PaletteControlBar::on_palette_locked_button_toggled(ToggleButton* button, PaletteControlBar* instance)
    {
        ...
        if (value == true)
        {
            instance->_palette_locked_toggle_button.set_child(&instance->_palette_locked_icon);
            instance->_palette_locked_toggle_button.set_tooltip_text("Palette Editing: Locked");
        }
        else
        {
            instance->_palette_locked_toggle_button.set_child(&instance->_palette_not_locked_icon);
            instance->_palette_locked_toggle_button.set_tooltip_text("Palette Editing: Active");
        }
    }
            ...
            : _owner(owner)
    {
        ...
        */
        ...
        _palette_locked_toggle_button.set_hexpand(false);
    ...
    {
        palette_locked = state::settings_file->get_value_as<bool>("palette_view.default", "palette_locked");
        tile_size = state::settings_file->get_value_as<float>("palette_view.default", "tile_size");
